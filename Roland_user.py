...
from utils.preprocess import load_graphs, get_context_pairs, get_evaluation_data,get_user
...
from models.roland import Roland
...
wandb.init(project="Roland", entity="houjinlin")
...
if __name__ == "__main__":
    ...
    model = Roland(args, feats[0].shape[1], 256,256).to(device)
    ...
    # model.eval()
    # emb = model(feed_dict["graphs"],feat)[-1].detach().cpu().numpy()
    ...
    wandb.log({"Best User AUC":best_epoch_test,"Best User AP":best_epoch_ap,"Best MAE":best_epoch_mae,"Best MSE":best_epoch_mse})
